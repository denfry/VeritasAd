...
class VeritasAdApiClient:
    ...
    def __init__(self, base_url: str, timeout: float = 90.0):
        ...
        logger.info(
            f"API Client initialized with base_url={self.base_url}, timeout={self.timeout}s"
        )
    ...
    async def analyze_url(self, api_key: str, url: str) -> dict:
        ...
        try:
            logger.debug(
                f"Sending POST request to {self.base_url}/api/v1/analyze/check"
            )
            ...
            logger.error(
                f"HTTP error analyzing URL: {e.response.status_code} - {e.response.text[:200] if e.response.text else 'No details'}"
            )
            raise
            ...
            logger.error(
                f"Unexpected error analyzing URL: {type(e).__name__} - {str(e)}"
            )
            raise
    ...
    async def analyze_file(self, api_key: str, filename: str, content: bytes) -> dict:
        ...
        try:
            ...
            logger.debug(
                f"Sending POST request to {self.base_url}/api/v1/analyze/check with file"
            )
            response = await client.post(
                f"{self.base_url}/api/v1/analyze/check",
                files=files,
            ...
            logger.error(
                f"HTTP error analyzing file: {e.response.status_code} - {e.response.text[:200] if e.response.text else 'No details'}"
            )
            raise
        except httpx.RequestError as e:
            ...
            logger.error(
                f"Unexpected error analyzing file: {type(e).__name__} - {str(e)}"
            )
            raise
    ...
    async def get_task_status(self, api_key: str, task_id: str) -> dict:
        ...
        try:
            ...
            result = response.json()
            logger.debug(
                f"Task {task_id} status: {result.get('status', 'unknown')} - {result.get('progress', 0)}%"
            )
            return result
        except httpx.HTTPStatusError as e:
            ...
            logger.error(
                f"Request error getting task status: {type(e).__name__} - {str(e)}"
            )
            raise
        except Exception as e:
            logger.error(
                f"Unexpected error getting task status: {type(e).__name__} - {str(e)}"
            )
            raise
    ...
    async def get_task_result(self, api_key: str, task_id: str) -> dict:
        ...
        try:
            ...
            result = response.json()
            logger.info(
                f"Task {task_id} result: status={result.get('status', 'unknown')}"
            )
            return result
        except httpx.HTTPStatusError as e:
            ...
            logger.error(
                f"Request error getting task result: {type(e).__name__} - {str(e)}"
            )
            raise
        except Exception as e:
            logger.error(
                f"Unexpected error getting task result: {type(e).__name__} - {str(e)}"
            )
            raise

    async def get_analysis_history(
        self, api_key: str, limit: int = 10, offset: int = 0
    ) -> list:
        """Get user's analysis history."""
        logger.debug(f"Getting analysis history: limit={limit}, offset={offset}")
        client = await self._get_client()
        try:
            response = await client.get(
                f"{self.base_url}/api/v1/analyze/history",
                params={"limit": limit, "offset": offset},
                headers={"X-API-Key": api_key, "X-Bot-Secret": settings.BOT_SECRET_KEY},
            )
            response.raise_for_status()
            result = response.json()
            logger.info(f"Retrieved {len(result)} history items")
            return result
        except httpx.HTTPStatusError as e:
            ...
            logger.error(
                f"Request error getting history: {type(e).__name__} - {str(e)}"
            )
            raise
        except Exception as e:
            logger.error(
                f"Unexpected error getting history: {type(e).__name__} - {str(e)}"
            )
            raise
